...
export const Leads = {
	getLead: async (id = false) => {
		let leads
		!id ? (leads = await LeadEntry.find({})) : (leads = await LeadEntry.find({ id: id }))
		return leads
	},
	...
	newLead: async (firstname, lastname, location, email, phone, make, model, issue) => {
		...
		try {
			const message = new Email(email, `Your ${C.brand} Repair Quote is Here!`, "quote", lead)
...
export const Devices = {
	getDevice: async (id = false) => {
		let devices
		!id ? (devices = await Pricelist.find({})) : (devices = await Pricelist.find({ id: id }))
